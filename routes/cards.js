...
router.put("/:id", auth, async (req, res) => {
    ...
    try {
        ...
        // add card to db
        let { companyName, companyEmail, imageBGC, description, image, address, phone } = req.body;
        // console.log("5555");
        console.log(companyEmail);

        Card.findOneAndUpdate({ id: req.params.id }, { companyName, companyEmail, imageBGC, description, image, address, phone }, { upsert: true, new: true })

        let update = await Card.findOneAndUpdate({ _id: req.params.id }, { companyName, companyEmail, imageBGC, description, image, address, phone }, { returnOriginal: false });
        await update.save();
